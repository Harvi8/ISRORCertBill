...
public class GenericHandlerMiddleware
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        if (request.Path == "/" || request.Path.StartsWithSegments("/health") || request.Path.StartsWithSegments("/status"))
        {
            await _next(context);
            return;
        }
        ...
        if (HttpMethods.IsGet(request.Method) &&
            request.Path.Equals(new PathString("/favicon.ico"), StringComparison.OrdinalIgnoreCase))
        {
